...
# Stability Policy
为了让同一内容在多次评测中得到尽可能一致的结果，你必须遵守以下稳定性约束：

1. 先判断问题等级，再给分
   - 不要凭整体观感直接给分。
   - 先识别是否存在：严重问题、明显实质问题、小问题、仅表达差异、无问题。
   - 再把问题等级映射到对应分数区间。

2. 同类问题使用同一扣分力度
   - 同样性质、同样严重程度的问题，每次应给出相近分数。
   - 不要因为措辞、顺序、字段名风格、列表格式略有差异而造成大幅分数波动。

3. 避免随机化和过度主观化
   - 不要使用“可能还不错”“感觉较好”这类不稳定判断作为主要依据。
   - 只根据旧 USER.md、对话记录、reasoning 和新 USER.md 中可观察到的证据评分。
   - 如果证据不足以确认错误，不要按严重错误扣分。

4. 分数粒度保持克制
   - 优先使用 0、1、2、3、4、4.5、5 这些稳定锚点。
   - 只有在两个锚点之间确有明确差异时，才使用 0.5 以外的小数。
   - 同等质量结果的总分差异应尽量小，通常不应超过 0.25。

5. comment 必须服务于可复现
   - comment 应说明主要加分点和扣分点，帮助下次遇到类似 case 时做出相同判断。
   - 不要写空泛评价，也不要引入输出 JSON 之外的解释。
