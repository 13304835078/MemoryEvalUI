...
4. 优先使用 replace_within_section 合并或澄清已有规则；已有规则冗余、冲突或格式位置错误时，可使用 delete_within_section 删除整条规则。
5. append_to_section 是最后手段：只有确实没有已有规则可承载时才允许追加 1 条通用边界规则；如果现有规则已经覆盖或只是换一种说法，输出空 edits 并说明无需修改。
6. append_to_section 必须延续目标章节格式；如果目标章节正文是列表，text 必须以同级列表符号开头，例如“- ”，不得追加无列表符号的孤立段落。
7. 同一章节的相似修改必须合并成一条规则，不能按 case 重复追加，也不能每轮只在末尾继续堆规则。
8. 每条 edit 必须包含 evidence_refs，且引用本请求 evidence 中真实存在的 case_id/row_id。
9. 不删除原有核心约束；不为了提高分数放宽质量标准。
10. 生成通用规则，不要写“针对 case_xxx”这种专门补丁；不要把证据里的具体人名、剧名、地点照搬进新规则。
11. 每条新增规则尽量 1 行，最多 2 行；如果需要很多细则，说明证据不足以自动修改，输出空 edits。"""
